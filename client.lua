...
CreateThread(function (threadId)
	...
	local weapon = lib.load('data.weapons').Weapons
	for hash in pairs(weapon) do
		WeaponMap[GetHashKey(hash)] = hash
	end
end)
...
local function registerCommands()
	...
	for i = 1, 5 do
		RegisterCommand(('hotbar%s'):format(i), function()
...
CreateThread(function()
	while true do
		local inventoryOpen = plyState.invOpen
		local sleep = inventoryOpen and 1000 or 0
		if IsControlJustReleased(1, 0xC1989F95) then -- I
			ExecuteCommand('inv')
		elseif IsControlJustReleased(1, keys[1]) then
			ExecuteCommand('hotbar1')
		elseif IsControlJustReleased(1, keys[2]) then
			ExecuteCommand('hotbar2')
		elseif IsControlJustReleased(1, keys[3]) then
			ExecuteCommand('hotbar3')
		elseif IsControlJustReleased(1, keys[4]) then
			ExecuteCommand('hotbar4')
		elseif IsControlJustReleased(1, keys[5]) then
			ExecuteCommand('hotbar5')
		elseif IsControlJustReleased(1, 0xE30CD707) then
				...
				elseif weaponAmmo then
					if IsPedShooting(playerPed) then
						...
						if currentAmmo <= 0 then
							...
							currentWeapon.timer = GetGameTimer() + 500
						end
					end
